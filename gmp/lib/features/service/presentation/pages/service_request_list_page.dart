...
class _ServiceRequestListPageState extends State<ServiceRequestListPage> {
  ...
  Future<void> _load() async {
    ...
    await tokenResult.fold(
      ...
      (token) async {
        try {
          final res = await sl<DioClient>().get(ApiEndpoints.serviceMy, accessToken: token);
          final list = ((res['data'] as Map<String, dynamic>?)?['requests'] as List?) ?? const [];
          if (!mounted) return;
          setState(() {
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : _error != null
              ...
              : _requests.isEmpty
                  ? const Center(
                      child: Text('No service requests yet', style: TextStyle(color: AppColors.textSecondary)),
                    )
                  : ListView.builder(
                        ...
                        return Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Text('Request: $id', style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
